...
def resp(mu,sig,pi,k,dim,X):
    ...
    for i in range(len(X)):
        ...
        for j in range(k):
            ...
            #multiplication of a number with a number
        for j in range(k):
...
def M(mu,sig,pi,k,dim,X,gamma):
    # >>> a = [1,2]
    # >>> b=[2,3]
    # >>> c = np.outer(np.array(a), np.array(b))
    # >>> print(c)
    # [[2 3]
    #  [4 6]]
    # >>> print(c.tolist())
    ...
    # >>> print(list(c))
    # [array([2, 3]), array([4, 6])]
    #this means to multiply two vectors to form matrix we must give the first arguement as the vector whose transpose
    # is to be taken(here implementation of vector is different than mathematical sense


    #do multiplication of matrix with vector
    # b=transposeVec((np.matmul(M, np.array(a)[np.newaxis].T)).tolist())
    # b is a list : shown as a vector (in terms of our representation) so that above multiplication can happen
    # M can be a list of list
    # a can be a list(), whose transpose we will take to perform matmul, it is a vector acc to our representation
    ...
    for i in range(k):
        ...
        for j in range(len(X)):
            ...
            # sig[i]+=gamma[j][i] * (X_mean_diff_T * X_mean_diff)) : matrix multiplication and multiplication
            # of scalar with matrix
...
def GMM(mu,sig,pi,X,k,dim):
    ...
    for z in range(100):
                ...
                if np.linalg.det(sig[i])==0:
                    for j in range(dim):
                        for ll in range(dim):
                            if j!=ll:
                                sig[i][j][ll]=0
        big='gmm_means_train.txt_'+str(z)
        arc = open(big, 'w')
        for i in range(len(mu)):
            ...
            arc.write("\n")
        arc.close()
        big2='3gmm_cov_train_.txt'+str(z)
        arc2 = open(big2, 'w')
...
mu,sig,pk=[],[],[]
f=open('3kmeans_train.txt','r')
g=open('3cov_train.txt','r')
h=open('3pk_train.txt','r')
for line in f:
...
def main():
    ...
    arc = open('3gmm_means_train_final.txt', 'w')
    for i in range(len(mu)):
        for j in range(len(mu[0])):
            arc.write(str(mu[i][j]))
            arc.write(' ')
        arc.write("\n")
    arc.close()
    arc2 = open('3gmm_cov_train_final.txt', 'w')
    for i in range(len(sig)):
        for j in range(dim):
            for q in range(dim):
                arc2.write(str(sig[i][j][q])+" ")
    ...
    arc3 = open('3gmm_pk_train.txt_final', 'w')
    for i in range(len(pi)):
        arc3.write(str(pi[i])+" ")
        arc3.write("\n")
    arc3.close()
