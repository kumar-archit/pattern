...
def N(X,mu,sig,k):
    ...
    try :
        ...
        print("Problem in calculating exponent of top, top1 : "+str(top1))
...
def GMM(mu,sig,pi,X,k,dim):
    ...
    for z in range(20):
                ...
                if np.linalg.det(sig[i])==0:
                    for j in range(dim):
                        for ll in range(dim):
                            if j!=ll:
                                sig[i][j][ll]=0
        arc = open('train_75/'+str(consideration)+'gmm_means_train.txt'+str(k), 'w')
        for i in range(len(mu)):
            ...
            arc.write("\n")
        arc.close()
        arc2 = open('train_75/'+str(consideration)+'gmm_cov_train.txt'+str(k), 'w')
        ...
        arc3 = open('train_75/'+str(consideration)+'gmm_pk_train.txt'+str(k), 'w')
        ...
        if abs(l_theta_old-l_theta_new)<0.01:
...
mu,sig,pk=[],[],[]
f=open('train_75/'+str(consideration)+'kmeans_train.txt'+str(k),'r')
g=open('train_75/'+str(consideration)+'cov_train.txt'+str(k),'r')
h=open('train_75/'+str(consideration)+'pk_train.txt'+str(k),'r')
for line in f:
