...
def main():
    ...
    xx = np.arange(-20,25,0.15)
    yy = np.arange(-20,25,0.15)
    ...
    for i in range(classes):
        ...
        for j in range(len(xx)):
            for q in range(len(yy)):
                Z[q][j]=N([xx[j],yy[q]],allMean[i],allCovFull[i],dim)#N(X,mu,sig,k)  #g_i(X,avg ,cov, k,pk)
