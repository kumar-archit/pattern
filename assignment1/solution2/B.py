...
k=0
def fn(X,k):
    return (-0.5*math.log(np.linalg.det(cov0))+math.log(n[k]*1.0/tot)-0.5*np.dot(np.dot(np.transpose(np.subtract(X,avg[k])),cinv[k]),np.subtract(X,avg[k]))-np.log(2*np.pi))
def g(i,j,cov1,cov2):
